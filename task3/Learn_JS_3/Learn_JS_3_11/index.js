...
// Обычно, когда сторона хочет закрыть соединение (браузер и сервер имеют равные права),
// они отправляют «фрейм закрытия соединения» с кодом закрытия и указывают причину в виде текста.
...
socket.onclose = (e) => {
  // e.code === 1000
  // e.reason === 'работа закончена'
  // e.wasClean === true (закрыто чисто)
};
...
// Коды WebSocket чем-то похожи на коды HTTP, но они разные. В частности, любые коды меньше 1000 зарезервированы.
// Если мы попытаемся установить такой код, то получим ошибку.
...
document.forms.publish.onsubmit = (e) => {
  e.preventDefault();
  const sendMessage = e.currentTarget.message.value;
...
socket.onmessage = (e) => {
  ...
  document.getElementById('messages').prepend(elem);
};
